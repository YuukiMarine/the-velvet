 ...
 *   ④ D0（低帧永久降级 / reduced-motion / 校直模式）下**定格**（paused），不逐帧推进。
 *      转场复刻份也短暂用过这条，两版都被用户否掉了，已撤回——见 frozen 的注释。
 *
...
interface BackgroundAnimationProps {
  ...
  darkMode?: boolean;
  /**
   * 定格：动画停在与活层同一相位（animation-play-state: paused），不再逐帧推进。
   * **现在只服务 D0**（低帧永久降级 / reduced-motion / 校直模式）——见 BackgroundAnimationInner。
   *
   * 转场复刻份曾经用过它，已撤回。两版都试过，两版都不对：
   *   v1 把 animation 整条摘掉 → 复刻份停在 0% 而活层在任意相位，实测差 39~58px，
   *      就是用户上报的「切页时背景闪一下」；
   *   v2 改成本字段（带相位定格，实测偏差 1.5px）→ 用户仍觉得突兀：定格只停播放、
   *      不停时间，垫层卸载那一下依然是「顿一下再跳到当前时刻」。
   * 复刻份现在原样跟着活层跑（v2.6.5 之前一直如此）。代价是擦除那 420ms 里
   * 合成动画数翻倍，但那是**一次性 420ms**，而闪烁是每次切页都看得见的。
   */
  frozen?: boolean;
}
