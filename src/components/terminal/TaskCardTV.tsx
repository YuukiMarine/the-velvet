/**
 * TaskCardTV — 24h 限时任务卡的 TV 表现层：一档「本期节目 · 倒计时」。
...
export const TaskCardTV = ({ vm }: { vm: TaskCardVM }) => {
  ...
  return (
    <motion.div layout initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
      <TVPanel title={<><RecDot size="h-1.5 w-1.5" bold={bold} />本期任务 · 24h</>} bodyClass={compact ? 'px-3 py-2.5' : 'px-3 py-3'}>
        <div className="flex items-center gap-2">
          {goalTitle && <span className="min-w-0 truncate text-[11px] text-white/55">本季《{goalTitle}》</span>}
        ...
        <div className="mt-2.5 flex items-center gap-2">
          <TVButton disabled={busy} onClick={onComplete} ariaLabel="我做到了">{busy ? '记下了…' : '✓ 通关'}</TVButton>
          <TVButton primary={false} onClick={requestDismiss} ariaLabel="放弃">放弃</TVButton>
        </div>
      </TVPanel>
    </motion.div>
  );
