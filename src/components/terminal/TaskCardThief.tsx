/**
 * TaskCardThief — 24h 当前小步卡的怪盗（红）表现层：一张「预告状」。
 *
  ...
  return (
    ...
    >
      ...
      <div className="relative mb-1.5 flex items-center gap-2">
        ...
        <span className="text-[11px] font-black tracking-[2px]" style={heavy(1.5)}>这一小步 · 24h</span>
        <span className={`ml-auto text-[11px] font-black tabular-nums ${expired ? 'text-amber-400' : 'text-primary'}`}>{countdown}</span>
      </div>

      {goalTitle && <div className="relative mb-0.5 truncate text-[11px] text-white/55">来自《{goalTitle}》</div>}
      ...
      <div className="relative mt-3 flex items-center gap-2">
        <StrikeButton label={busy ? '记下了…' : '我做到了'} onClick={onComplete} disabled={busy} mask live={!compact} className="flex-1" />
        <GhostButton label="先放着" onClick={requestDismiss} />
      </div>
    </motion.div>
  );
};
