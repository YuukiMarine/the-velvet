/**
 * TaskCardBoard — 24h 当前小步卡的讨论板表现层：一个「置顶帖」。
...
export const TaskCardBoard = ({ vm }: { vm: TaskCardVM }) => {
  ...
  return (
    <motion.div layout initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
      <BevelWindow title="[置顶] 这一小步 · 24h" bodyClass={compact ? 'px-3 py-2.5' : 'px-3 py-3'}>
        <div className={compact ? 'text-[12px]' : 'text-[13px]'}>
          ...
          {goalTitle && <div className="mt-1 truncate" style={{ color: INK_DIM }}>来自《{goalTitle}》</div>}
          ...
          <div className="mt-2.5 flex gap-2">
            ...
            <BevelButton onClick={requestDismiss} ariaLabel="先放着">先放着</BevelButton>
          </div>
        </div>
      </BevelWindow>
    </motion.div>
