...
export function Cooperation() {
  ...
  return (
    ...
    >
      ...
      {/* 浮动新增按钮（p3：右下大青斜块 +，设计稿） */}
      {remaining > 0 && filter !== 'archived' && (
        <motion.button
          whileTap={{ scale: 0.95 }}
          onClick={() => setCreateOpen(true)}
          className={p3
            ? 'fixed bottom-24 md:bottom-8 right-5 md:right-8 z-40 flex h-14 w-[76px] items-center justify-center text-3xl font-black text-white'
            : 'fixed bottom-24 md:bottom-8 right-5 md:right-8 z-40 w-14 h-14 rounded-full text-white shadow-2xl flex items-center justify-center text-2xl font-bold'}
          style={p3 ? {
