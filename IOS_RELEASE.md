...
- [ ] 主屏加「今日/征途/清单/牌与月」小组件 + 锁屏加「今日/征途/清单」扁条，抽一张塔罗、勾一个任务后确认组件跟着刷新
- [ ] 「清单」组件：建一件带截止日的 BIG DEAL + 一个「重要」任务，确认蓝板倒计时（≤2 天转洋红）与琥珀高亮都在；2×2 与 4×2 两个尺寸各看一眼，钉一张带目标日的宣告卡确认 4×2 的「剩 N 天」行
